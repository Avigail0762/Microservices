...
Write-Host '== STEP 0: Recreate stack with 2 catalog replicas =='
...
Write-Host '== STEP 2: Gateway health =='
$health = curl.exe -s -i http://localhost:8080/health
$health
if ($health -match '200 OK') { Write-Host 'PASS: gateway health OK' } else { throw 'Gateway health failed' }
